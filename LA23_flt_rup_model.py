...
def LavrentiadisAbrahamson2023SlipProfile(x_array, mag, srl, sof='Strike-Slip'):
    ...
    sig_agg : np.array()
        Total aggregate aleatory variability.
    sig_prnc : np.array()
        Total principal aleatory variability.
     phi_agg : np.array()
    ...
    #general agg slip
    #---   ---   ---   ---   ---
    d_agg = c_0 + c_1 * (xl_array-0.3) + c_2 * (xl_array-0.3)**2 + c_3 * (mag-7.0)
    d_agg = np.exp(d_agg)
    ...
    T_M = np.maximum( np.minimum((7.-mag)/(7.-M_1), 1.), 0. )
    
    #median slip (pwr=0.3)
    #---   ---   ---   ---   ---
    mu_agg_seg = (d_agg*np.exp(c_5*T_xl))**0.3 + c_6*T_M + c_7
    ...
    f_NDmu = ( c_11*xl_offset1    + c_12*xl_offset1**2 + 
               c_13*xl_offset1**3 + c_14*xl_offset1**4 +
               c_14a*xl_offset2 )
    f_NDmu = c_10 + f_NDmu
    #amplitude effect
    ...
    #shape normalization
    f_NPGap  = 20.0     * c_24          * np.minimum( np.maximum(xl_array-0.10,0.0), 0.05)  #first and second leg
    f_NPGap += 0.13**-1 * (1.0  - c_24) * np.minimum( np.maximum(xl_array-0.15,0.0), 0.13)  #third leg and fourth
    f_NPGap -= 10.0     * (1.0  - c_25) * np.minimum( np.maximum(xl_array-0.30,0.0), 0.10)  #fifth leg
    f_NPGap -= 10.0     * (c_25 - c_26) * np.minimum( np.maximum(xl_array-0.40,0.0), 0.10)  #sixth leg
    ...
    mu_prnc_prime = np.maximum(mu_prnc_prime, 0.)
    ...
    return disp_agg_prime, disp_prnc_prime, disp_agg_seg, sig_agg, sig_prnc, phi_agg, phi_prnc, tau_agg, phi_add, P_gap, P_zero_slip    
...
def LavrentiadisAbrahamson2023SlipProfilePrc(x_array, mag, srl, sof='Strike-Slip', prc=0.5):
    ...
    disp_agg_p_prc[disp_agg_p_prc<0] = 0.
    ...
    disp_prnc_p_prc[disp_prnc_p_prc<0] = 0.
    disp_prnc_p_prc = disp_prnc_p_prc**(1/0.3) 
    
    return disp_agg_p_prc, disp_prnc_p_prc
